...
def draw_pose_keypoints(image, keypoints, validation_results=None):
    ...
    for person_idx, person_keypoints in enumerate(keypoints):
        ...
        if validation_results and person_idx < len(validation_results):
            ...
            cv2.putText(image, f"Shoulders: {shoulder_status}", (10, 30 + person_idx * 180), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, shoulder_color, 2)
            cv2.putText(image, f"Dist: {person_results['shoulders']['horizontal_distance']:.1f}%", 
                       (10, 55 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, shoulder_color, 1)
            ...
            if "left" in person_results and person_results["left"]["vertical_percent"] != 0:
                ...
                cv2.putText(image, f"Left Wrist: {left_status}", (10, 80 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, left_color, 2)
                cv2.putText(image, f"Vert: {person_results['left']['vertical_percent']:.1f}%", 
                           (10, 105 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, left_color, 1)
            ...
            if "right" in person_results and person_results["right"]["vertical_percent"] != 0:
                ...
                cv2.putText(image, f"Right Wrist: {right_status}", (200, 80 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, right_color, 2)
                cv2.putText(image, f"Vert: {person_results['right']['vertical_percent']:.1f}%", 
                           (200, 105 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, right_color, 1)
            
            # Draw elbow validation
            if "left_elbow" in person_results and person_results["left_elbow"]["vertical_percent"] != 0:
                left_elbow_status = "PASS" if person_results["left_elbow"]["valid"] else "FAIL"
                left_elbow_color = valid_color if person_results["left_elbow"]["valid"] else invalid_color
                cv2.putText(image, f"Left Elbow: {left_elbow_status}", (10, 130 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, left_elbow_color, 2)
                cv2.putText(image, f"Vert: {person_results['left_elbow']['vertical_percent']:.1f}%", 
                           (10, 155 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, left_elbow_color, 1)
            
            if "right_elbow" in person_results and person_results["right_elbow"]["vertical_percent"] != 0:
                right_elbow_status = "PASS" if person_results["right_elbow"]["valid"] else "FAIL"
                right_elbow_color = valid_color if person_results["right_elbow"]["valid"] else invalid_color
                cv2.putText(image, f"Right Elbow: {right_elbow_status}", (200, 130 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, right_elbow_color, 2)
                cv2.putText(image, f"Vert: {person_results['right_elbow']['vertical_percent']:.1f}%", 
                           (200, 155 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, right_elbow_color, 1)
            
            # Draw knee validation
            if "left_knee" in person_results and person_results["left_knee"]["vertical_percent"] != 0:
                left_knee_status = "PASS" if person_results["left_knee"]["valid"] else "FAIL"
                left_knee_color = valid_color if person_results["left_knee"]["valid"] else invalid_color
                cv2.putText(image, f"Left Knee: {left_knee_status}", (10, 180 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, left_knee_color, 2)
                cv2.putText(image, f"Vert: {person_results['left_knee']['vertical_percent']:.1f}%", 
                           (10, 205 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, left_knee_color, 1)
            
            if "right_knee" in person_results and person_results["right_knee"]["vertical_percent"] != 0:
                right_knee_status = "PASS" if person_results["right_knee"]["valid"] else "FAIL"
                right_knee_color = valid_color if person_results["right_knee"]["valid"] else invalid_color
                cv2.putText(image, f"Right Knee: {right_knee_status}", (200, 180 + person_idx * 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, right_knee_color, 2)
                cv2.putText(image, f"Vert: {person_results['right_knee']['vertical_percent']:.1f}%", 
                           (200, 205 + person_idx * 180), cv2.FONT_HERSHEY_SIMPLEX, 0.5, right_knee_color, 1)
    ...
    while True:
        if not paused:
            ...
            for result in results:
                ...
                # Validate pose positions for each person
                for person_idx, person_kpts in enumerate(adjusted_keypoints):
                    validation_results = validate_pose_positions(
                        person_kpts, 
                        min_wrist_percent, 
                        max_wrist_percent,
                        min_elbow_percent,
                    ...
                    # Now including elbow and knee validation in the condition
                    if (validation_results["shoulders"]["valid"] and 
                        ((wrist_type == "both" and validation_results["left"]["valid"] and validation_results["right"]["valid"]) or
                         ...
                         (wrist_type == "right" and validation_results["right"]["valid"])) and
                        (validation_results["left_elbow"]["valid"] or validation_results["right_elbow"]["valid"]) and
                        (validation_results["left_knee"]["valid"] or validation_results["right_knee"]["valid"])):
